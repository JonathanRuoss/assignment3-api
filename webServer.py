...
class BmiCalculatorController(Resource):
    def post(self):
        ...
        calculator = BmiCalculator(
            requestParams["heightFt"],
            requestParams["heightIn"],
            requestParams["weight"]
            )
        return {
            'value': calculator.getBmiVal(),
            'category': calculator.getBmiCat()
            }, 200
...
class RetirementCalculatorController(Resource):
    def post(self):
        ...
        calculator = RetirementCalculator(
            ...
            requestParams["percentage"],
            requestParams["goal"]
            )
        return {
            'age': calculator.getFinalAge(),
            'goalMet': calculator.getGoalMet()
            }, 200
